...
4. The object returned from `returned `hclust1d` call is the same S3 class as the result of `stats::hclust`, namely `hclust` S3 class.
...
# Replacing `stats::hclust` in case of all other linkage functions, beside `ward.D`, `centroid` or `median`
